...
export class ControlPlaneClient {
  ...
  async register(payload: Record<string, unknown>): Promise<void> {
    const response = await fetch(new URL('/internal/nodes/register', this.config.CONTROL_PLANE_URL), {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-bootstrap-token': this.config.NODE_BOOTSTRAP_TOKEN },
      body: JSON.stringify(payload),
    });
    if (!response.ok) throw new Error(`Node registration failed: ${response.status} ${await response.text()}`);
  }

  async request<T>(method: string, path: string, body?: unknown): Promise<T> {
    const rawBody = body === undefined ? new Uint8Array() : new TextEncoder().encode(JSON.stringify(body));
    const response = await this.signedFetch(method, path, rawBody, body === undefined ? undefined : 'application/json');
    if (!response.ok) throw new Error(`Control plane ${method} ${path} failed: ${response.status} ${await response.text()}`);
  ...
  async uploadArtifact(taskId: string, attemptId: string, input: { bytes: Uint8Array; mime: string; kind: string; name: string }): Promise<ArtifactRef> {
  ...
  async event(taskId: string, attemptId: string, event: string, data: Record<string, unknown> = {}): Promise<void> {
    await this.request('POST', `/internal/tasks/${taskId}/event`, { attemptId, event, data });
  }

  async complete(taskId: string, input: { attemptId: string; engine: ExecutedEngine; resultArtifactId?: string; durationMs: number; memoryPeakMb?: number; confidence?: number; fallbackChain: string[] }): Promise<void> {
    await this.request('POST', `/internal/tasks/${taskId}/complete`, input);
  }

  async fail(taskId: string, input: { attemptId: string; errorCode: string; errorMessage: string; retryable: boolean; durationMs: number; memoryPeakMb?: number }): Promise<void> {
  ...
  private async signedFetch(method: string, path: string, body: Uint8Array, contentType?: string, extraHeaders: Record<string, string> = {}): Promise<Response> {
    const timestamp = String(Date.now());
    const nonce = randomUUID();
    const bodyHash = sha256(body);
    const signature = signHmac(this.config.NODE_SECRET, { method, path, bodyHash, timestamp, nonce });
