...
async function canAccessTask(c: AppContext, taskId: string): Promise<boolean> {
...
app.get('/health', (c) =>
  c.json({ status: 'ok', service: 'feather-runtime-control-plane', time: Date.now() }),
);

app.post('/internal/nodes/register', async (c) => {
  if (c.req.header('x-bootstrap-token') !== c.env.BOOTSTRAP_TOKEN)
  ...
  await c.env.DB.prepare(
    `INSERT INTO nodes(id,hostname,agent_version,architecture,cpu_count,memory_mb,last_seen,status,capabilities_json,hmac_key_enc,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET hostname=excluded.hostname,agent_version=excluded.agent_version,architecture=excluded.architecture,cpu_count=excluded.cpu_count,memory_mb=excluded.memory_mb,last_seen=excluded.last_seen,capabilities_json=excluded.capabilities_json,hmac_key_enc=excluded.hmac_key_enc,updated_at=excluded.updated_at`,
  )
    .bind(
      input.nodeId,
      input.hostname,
...
app.post('/internal/bootstrap/api-keys', async (c) => {
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(
      'INSERT INTO api_keys(id,user_id,name,token_hash,scopes,created_at) VALUES(?,?,?,?,?,?)',
    ).bind(keyId, input.userId, input.name, tokenHash, input.scopes.join(' '), now),
    c.env.DB.prepare(
      'INSERT INTO audit_logs(user_id,actor_type,actor_id,action,target_type,target_id,data_json,created_at) VALUES(?,?,?,?,?,?,?,?)',
    ).bind(
      input.userId,
      'bootstrap',
      null,
      ...
      'api_key',
      keyId,
      JSON.stringify({ name: input.name, scopes: input.scopes }),
      now,
    ),
  ]);
...
app.post('/internal/nodes/heartbeat', async (c) => {
  const input = z
    .object({
      ...
      cpuLoad: z.number().nonnegative(),
      memoryUsedMb: z.number().nonnegative(),
      activeTasks: z.number().int().nonnegative(),
      lightpandaHealthy: z.boolean(),
      chromiumRunning: z.number().int().nonnegative(),
      version: z.string(),
    })
    .parse(await c.req.json());
  if (input.nodeId !== c.get('actorId')) return c.json({ error: 'node_mismatch' }, 403);
  await c.env.DB.prepare(
    'UPDATE nodes SET last_seen=?,status=?,agent_version=?,capabilities_json=?,updated_at=? WHERE id=?',
  )
    .bind(
      Date.now(),
      input.status,
      ...
      JSON.stringify({
        ...
        lightpandaHealthy: input.lightpandaHealthy,
        chromiumRunning: input.chromiumRunning,
      }),
      Date.now(),
      input.nodeId,
    )
...
app.get('/internal/tasks/:id', async (c) => {
  ...
  const policy = hostname
    ? await c.env.DB.prepare('SELECT * FROM domain_policies WHERE domain=?')
        .bind(hostname)
...
app.post('/internal/tasks/:id/start', async (c) => {
  ...
  const input = z
    .object({
      ...
      engine: z.enum(['http', 'document', 'lightpanda', 'chromium', 'ocr']),
    })
    .parse(await c.req.json());
  const task = await c.env.DB.prepare(
    'SELECT status,destructive,cancel_requested FROM tasks WHERE id=?',
  )
    .bind(taskId)
    .first<{ status: string; destructive: number; cancel_requested: number }>();
  if (!task) return c.json({ error: 'not_found' }, 404);
  if (task.cancel_requested === 1 || task.status === 'cancelled')
    return c.json({ execute: false, reason: 'cancelled' });
  if (task.status === 'created')
    return c.json({ execute: false, reason: 'not_queued_yet', retry: true });
  const attempt = await c.env.DB.prepare(
    'SELECT status,idempotency_key FROM task_attempts WHERE id=? AND task_id=?',
  )
    .bind(input.attemptId, taskId)
    .first<{ status: string; idempotency_key: string }>();
  if (!attempt || attempt.idempotency_key !== input.idempotencyKey)
    return c.json({ error: 'invalid_attempt' }, 409);
  if (attempt.status !== 'queued' || task.status !== 'queued')
    return c.json({ execute: false, reason: 'already_leased' });
  const now = Date.now();
  if (task.destructive === 1) {
    try {
      await c.env.DB.prepare(
        'INSERT INTO destructive_idempotency(idempotency_key,task_id,attempt_id,state,created_at,updated_at) VALUES(?,?,?,?,?,?)',
      )
        .bind(input.idempotencyKey, taskId, input.attemptId, 'reserved', now, now)
        .run();
    } catch {
  ...
  const leased = await c.env.DB.prepare(
    "UPDATE tasks SET status='leased' WHERE id=? AND status='queued'",
  )
    .bind(taskId)
    .run();
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(
      "UPDATE tasks SET status='running',started_at=COALESCE(started_at,?),selected_engine=?,attempt_count=attempt_count+1 WHERE id=? AND status='leased'",
    ).bind(now, input.engine, taskId),
    c.env.DB.prepare(
      "UPDATE task_attempts SET status='running',node_id=?,engine=?,started_at=? WHERE id=? AND status='queued'",
    ).bind(c.get('actorId'), input.engine, now, input.attemptId),
    c.env.DB.prepare(
      'INSERT INTO task_events(task_id,attempt_id,node_id,event,data_json,created_at) VALUES(?,?,?,?,?,?)',
    ).bind(
      taskId,
      input.attemptId,
      c.get('actorId'),
      'task.started',
      JSON.stringify({ engine: input.engine }),
      now,
    ),
  ]);
...
app.post('/internal/tasks/:id/attempts/:attemptId/artifacts', async (c) => {
  ...
  const filename = (c.req.header('x-artifact-name') ?? 'artifact.bin').replace(
    /[^a-zA-Z0-9._-]+/g,
    '_',
  );
  const artifactId = `art_${crypto.randomUUID().replaceAll('-', '')}`;
  const owner = await c.env.DB.prepare('SELECT user_id FROM tasks WHERE id=?')
    .bind(taskId)
  ...
  await c.env.ARTIFACTS.put(key, bytes, {
    ...
    customMetadata: { taskId, attemptId, artifactId, sha256: hash },
  });
  await c.env.DB.batch([
    c.env.DB.prepare(
      'INSERT INTO artifacts(id,task_id,attempt_id,kind,r2_key,sha256,size,mime,created_at) VALUES(?,?,?,?,?,?,?,?,?)',
    ).bind(artifactId, taskId, attemptId, kind, key, hash, bytes.byteLength, mime, Date.now()),
    c.env.DB.prepare(
      'INSERT INTO task_events(task_id,attempt_id,node_id,event,data_json,created_at) VALUES(?,?,?,?,?,?)',
    ).bind(
      taskId,
      ...
      c.get('actorId'),
      'artifact.created',
      JSON.stringify({ artifactId, kind, size: bytes.byteLength, mime }),
      Date.now(),
    ),
  ]);
...
app.get('/internal/artifacts/:id', async (c) => {
  const row = await c.env.DB.prepare('SELECT r2_key,mime,sha256,size FROM artifacts WHERE id=?')
    .bind(c.req.param('id'))
    .first<{ r2_key: string; mime: string; sha256: string; size: number }>();
...
app.post('/internal/tasks/:id/complete', async (c) => {
  ...
  const updated = await c.env.DB.prepare(
    "UPDATE tasks SET status='completed',selected_engine=?,finished_at=?,result_artifact_id=?,error_code=NULL,error_message=NULL WHERE id=? AND status='running'",
  )
    .bind(input.engine, now, input.resultArtifactId ?? null, taskId)
    .run();
  if ((updated.meta.changes ?? 0) !== 1) {
    const current = await c.env.DB.prepare('SELECT status FROM tasks WHERE id=?')
      .bind(taskId)
      .first<{ status: string }>();
    return c.json(
      { ok: current?.status === 'completed', status: current?.status ?? 'missing' },
      current?.status === 'completed' ? 200 : 409,
    );
  }
  await c.env.DB.batch([
    c.env.DB.prepare(
      "UPDATE task_attempts SET status='completed',finished_at=?,duration_ms=?,memory_peak_mb=?,result_artifact_id=? WHERE id=? AND task_id=?",
    ).bind(
      now,
      input.durationMs,
      input.memoryPeakMb ?? null,
    ...
    c.env.DB.prepare(
      "UPDATE destructive_idempotency SET state='completed',updated_at=? WHERE task_id=? AND attempt_id=?",
    ).bind(now, taskId, input.attemptId),
  ]);
  return c.json({ ok: true });
});
...
app.post('/internal/tasks/:id/fail', async (c) => {
  ...
  const input = z
    ...
    .parse(await c.req.json());
  const task = await c.env.DB.prepare(
    'SELECT status,destructive,attempt_count,max_attempts FROM tasks WHERE id=?',
  )
    .bind(taskId)
    .first<{ status: string; destructive: number; attempt_count: number; max_attempts: number }>();
  if (!task) return c.json({ error: 'not_found' }, 404);
  ...
  const changed = await c.env.DB.prepare(
    `UPDATE tasks SET status=?,finished_at=${canRetry ? 'NULL' : '?'},error_code=?,error_message=? WHERE id=? AND status='running'`,
  )
    .bind(
      ...(canRetry
        ? [nextStatus, input.errorCode, input.errorMessage, taskId]
        : [nextStatus, now, input.errorCode, input.errorMessage, taskId]),
    )
    .run();
  if ((changed.meta.changes ?? 0) !== 1)
    return c.json({ error: 'invalid_state', status: task.status }, 409);
  await c.env.DB.batch([
    c.env.DB.prepare(
      "UPDATE task_attempts SET status='failed',finished_at=?,duration_ms=?,error_code=?,error_message=?,memory_peak_mb=? WHERE id=? AND task_id=?",
    ).bind(
      now,
      input.durationMs,
      input.errorCode,
    ...
    c.env.DB.prepare(
      ...
      input.attemptId,
      c.get('actorId'),
      canRetry ? 'task.retrying' : 'task.failed',
      JSON.stringify({ errorCode: input.errorCode, retryable: input.retryable }),
      now,
    ),
    c.env.DB.prepare(
      "UPDATE destructive_idempotency SET state='unknown',updated_at=? WHERE task_id=? AND attempt_id=? AND state='reserved'",
    ).bind(now, taskId, input.attemptId),
  ]);
  if (canRetry) {
...
app.post('/v1/scrape', requireScope('tasks:write'), async (c) => {
  const input = z
    .object({
      ...
      engine: z.enum(['auto', 'http', 'lightpanda', 'chromium']).default('auto'),
...
app.post('/v1/documents', requireScope('tasks:write'), async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind(artifactId, taskId, null, 'source', key, hash, bytes.byteLength, mime, now)
    .run();
  ...
  await c.env.JOBS.send(
    { version: 1, taskId, attemptId, idempotencyKey, priority: 0, createdAt: now },
    { contentType: 'json' },
  );
...
app.post('/v1/tasks/:id/cancel', requireScope('tasks:write'), async (c) => {
  ...
  const row = await c.env.DB.prepare('SELECT status FROM tasks WHERE id=?')
    .bind(taskId)
    .first<{ status: string }>();
  if (!row) return c.json({ error: 'not_found' }, 404);
  ...
  await c.env.DB.prepare(
    "UPDATE tasks SET status='cancelled',cancel_requested=1,finished_at=? WHERE id=? AND status IN ('created','queued','retrying')",
  )
...
app.get('/v1/tasks/:id/events', requireScope('tasks:read'), async (c) => {
  if (!(await canAccessTask(c, c.req.param('id')!))) return c.json({ error: 'not_found' }, 404);
  const rows = await c.env.DB.prepare('SELECT * FROM task_events WHERE task_id=? ORDER BY id ASC')
    .bind(c.req.param('id'))
    .all();
...
app.get('/v1/tasks/:id/artifacts', requireScope('artifacts:read'), async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    'SELECT id,task_id,attempt_id,kind,r2_key,sha256,size,mime,created_at FROM artifacts WHERE task_id=? ORDER BY created_at ASC',
  )
    .bind(c.req.param('id'))
    .all();
  return c.json(rows.results);
...
app.get('/v1/nodes', requireScope('admin'), async (c) => {
  const rows = await c.env.DB.prepare(
    'SELECT id,hostname,agent_version,architecture,cpu_count,memory_mb,last_seen,status,capabilities_json,created_at,updated_at FROM nodes ORDER BY last_seen DESC',
  ).all();
  return c.json(rows.results);
});
...
app.onError((error, c) => {
  ...
  if (error instanceof z.ZodError)
    return c.json({ error: 'invalid_input', issues: error.issues }, 400);
